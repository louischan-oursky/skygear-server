...
func InjectAuthorizeHandler(r *http.Request, dep *inject.BootTimeDependency) *AuthorizeHandler {
	...
	pqStoreImpl := ProvideAuthInfoStore(tenantConfiguration, sqlExecutor)
	...
	redisStoreImpl := ProvideSessionStore(tenantConfiguration, context, providerImpl, factoryImpl)
	eventStoreImpl := ProvideSessionEventStore(tenantConfiguration, context)
	sessionProviderImpl := ProvideSessionProvider(r, tenantConfiguration, redisStoreImpl, eventStoreImpl, authContextProviderImpl, providerImpl)
	customtokenProviderImpl := ProvideCustomTokenProvider(sqlBuilder, sqlExecutor, tenantConfiguration)
	oauthProviderImpl := oauth.NewProvider(sqlBuilder, sqlExecutor)
	identityProviderImpl := ProvideIdentityProvider(sqlBuilder, sqlExecutor, passwordProviderImpl, customtokenProviderImpl, oauthProviderImpl)
	modelproviderProviderImpl := modelprovider.NewProvider(pqStoreImpl, userprofileStoreImpl, identityProviderImpl)
	redisStore := ProvideAuthorizationCodeStore(context, tenantConfiguration, providerImpl)
	useInsecureCookie := ProvideUseInsecureCookie(dep)
	authenticationProviderImpl := provider.NewAuthenticationProvider(passwordProviderImpl, trail, factoryImpl, providerImpl, authContextProviderImpl, mfaProviderImpl, tenantConfiguration, pqStoreImpl, hookProviderImpl, sessionProviderImpl, modelproviderProviderImpl, redisStore, useInsecureCookie)
...
func ProvideAuthInfoStore(
	...
	return pq.NewAuthInfoStore(db.NewSQLBuilder("core", tConfig.DatabaseConfig.DatabaseSchema, tConfig.AppID), sqlExecutor)
}
